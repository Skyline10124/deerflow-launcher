...
export class ProcessMonitor {
  ...
  async connect(): Promise<void> {
    this.connected = true;
    this.logger.debug('ProcessMonitor connected (Bun native)');
  }
  ...
  startMonitoring(services: ServiceName[]): void {
    ...
    for (const service of services) {
      this.serviceStartTimes.set(service, new Date());
    }
  ...
  private async checkProcesses(services: ServiceName[]): Promise<void> {
    if (!this.connected || !this.processManager) return;
    ...
    try {
      ...
      for (const serviceName of services) {
        ...
        if (proc.status === 'crashed') {
          await this.handleProcessError(serviceName);
  ...
  private async handleProcessError(serviceName: string): Promise<void> {
    ...
    if (this.onServiceError) {
      this.onServiceError(serviceName, new Error(`Process crashed`));
    }
  }
  ...
  async getStatus(): Promise<ProcessStatus[]> {
    ...
    return processes.map(proc => {
      ...
      const startTime = this.serviceStartTimes.get(proc.name);
      const uptime = startTime ? Date.now() - startTime.getTime() : 0;
      
      return {
        name: proc.name,
        status: this.mapStatus(proc.status),
        cpu,
        memory,
        restarts: this.restartAttempts.get(proc.name) || 0,
        uptime,
  ...
  private mapStatus(status: string): ProcessStatus['status'] {
    switch (status) {
      case 'running':
        return 'online';
      case 'stopped':
