...
async function main() {
  ...
  try {
    ...
    const handleInstanceSwitch = (newInstanceName: string) => {
      logger.info(`Switching to instance: ${newInstanceName}`);

      const paths = getDeerflowPaths();
      const targetPath = paths.find(p => p.name === newInstanceName);

      if (!targetPath) {
        logger.error(`Instance not found: ${newInstanceName}`);
        return;
      }
      ...
      cleanup().then(() => {
        ...
        const env: Record<string, string> = {
          ...process.env,
          DEERFLOW_PATH: targetPath.path,
          INSTANCE_ID: newInstanceName,
          NO_MONITOR: noMonitor ? 'true' : 'false',
        };
